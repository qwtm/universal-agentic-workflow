...
| Build Record | `uwf-br` | Operational | SQLite | `uwf-core-blueprint` agent (initializes layers 0–4), all stages (each appends to its layer), `uwf-core-snapshot` agent (closes layer 5) | Append-only layered execution log. Six layers: 0 (context), 1 (decisions), 2 (dependencies), 3 (actions), 4 (verification), 5 (state). One SQLite table per layer; append-only enforced by DDL triggers. At `snapshot` stage, exported as `uwf-drs` JSON. Schema: `docs/artifacts/uwf-br.md`. |
